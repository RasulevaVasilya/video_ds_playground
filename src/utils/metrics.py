...
def compare_wer_table(results: dict[str, dict]) -> str:
    ...
    # Сортируем, обрабатывая None значения
    def get_wer(item):
        data = item[1]
        wer_val = data.get("wer")
        if wer_val is None:
            return 999.0  # None отправляем в конец
        return wer_val
    
    for model, data in sorted(results.items(), key=get_wer):
        wer_val = data.get("wer")
        ...
        if wer_val is None:
            ...
            wer_str = f"{wer_val:>8.4f}"
        
        lines.append(f"{model:<30} {wer_str} {time_val:>12.1f}")
